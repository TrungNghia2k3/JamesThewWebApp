...
// Separate component for parameter input to prevent focus loss
const ParameterInput = React.memo(({ param, value, onChange, validationErrors }) => (
    <div>
        <label className="block text-sm font-medium text-gray-700 capitalize mb-1">
...
const DynamicRequestForm = React.memo(({ 
    currentEndpoint, 
    requestData, 
    updateRequestField, 
    updateNestedField 
    ...
    // Check if we should render forms
    const hasRequestBody = currentEndpoint?.requestBody && ['POST', 'PUT', 'DELETE'].includes(currentEndpoint.method);
    const hasParameters = currentEndpoint?.parameters && currentEndpoint.method === 'GET' && currentEndpoint.parameters.length > 0;

    const handleFileChange = React.useCallback((field, file) => {
    ...
    }, [updateRequestField, currentEndpoint?.validation]);
    ...
    const getMethodTip = React.useCallback((method) => {
        const tips = {
    ...
    const getContentTypeTip = React.useCallback(() => {
    ...
    const enhancedUpdateRequestField = React.useCallback((field, value) => {
        handleFieldChange(field, value);
    }, [handleFieldChange]);

    // Component for rendering GET parameters
    const ParametersForm = React.useMemo(() => {
            ...
            <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="space-y-3">
                    {currentEndpoint.parameters.map((param) => (
                        <ParameterInput
                            ...
                            onChange={(e) => updateRequestField(param.name, e.target.value)}
                            validationErrors={validationErrors[param.name]}
                        />
                    ))}
                </div>
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded">
                    <p className="text-xs text-blue-700">
                        ...
                        These parameters will be added to the URL as query string parameters (e.g., ?id=1&page=2). Values are pre-populated from the Sample Request - you can modify them as needed.
                    </p>
                </div>
            </div>
        );
    }, [hasParameters, currentEndpoint?.parameters, requestData, updateRequestField, validationErrors]);

    // Component for rendering request body form
    const RequestBodyForm = React.useMemo(() => {
    ...
    }, [hasRequestBody, currentEndpoint, requestData, enhancedUpdateRequestField, updateNestedField, handleFileChange, validationErrors, getMethodTip, getContentTypeTip]);

    // Early return after all hooks - if no endpoint is selected or no forms to render
    if (!currentEndpoint || (!hasRequestBody && !hasParameters)) {
        return null;
    }
